...
import type { Book, BookBorrowing } from '../../types'
...
function MyBorrowings() {
    ...
    const currentBorrowings = myBorrowings.filter((b: BookBorrowing) => b.status === 'borrowed')
    const historyBorrowings = myBorrowings.filter((b: BookBorrowing) => b.status === 'returned')

    const getBookInfo = (bookId: number, borrowing?: BookBorrowing): BookInfo => {
        const found = books.find((b: Book) => b.id === bookId)
        if (found) return found as BookInfo
        return {
        ...
        <div className="page-container">
            ...
            {activeTab === 'current' && (
                <div className="borrowing-list">
                    {currentBorrowings.map((borrowing: BookBorrowing) => {
            ...
            {activeTab === 'history' && (
                <div className="data-table-container">
                    <table className="data-table">
                        ...
                        <tbody>
                            {historyBorrowings.map((borrowing: BookBorrowing) => {
                                const book = getBookInfo(borrowing.bookId, borrowing)
                                return (
                                    <tr key={borrowing.id}>

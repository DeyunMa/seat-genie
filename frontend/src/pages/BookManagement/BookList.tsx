...
function BookList() {
    ...
    const loadBooks = useCallback(async () => {
        ...
        try {
            ...
            addToast((error as Error).message || '获取图书失败', 'error')
        } finally {
            setLoading(false)
        }
    }, [addToast])
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            addToast((error as Error).message || '保存图书失败', 'error')
        }
    ...
    const handleDelete = async () => {
        if (selectedBookId) {
            try {
                ...
                addToast((error as Error).message || '删除图书失败', 'error')
            } finally {

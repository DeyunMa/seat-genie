...
import type { Seat, Room, SeatReservation } from '../../types'
...
function MyReservations() {
    ...
    const myReservations = seatReservations.filter((r: SeatReservation) => r.userId === user?.id)
    const today = new Date().toISOString().split('T')[0]
    const now = new Date()
    const currentTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`

    const activeReservations = myReservations.filter((r: SeatReservation) =>
        r.status === 'active' &&
        (r.date > today || (r.date === today && r.endTime > currentTime))
    )

    const historyReservations = myReservations.filter((r: SeatReservation) =>
        r.status !== 'active' ||
        r.date < today ||
        (r.date === today && r.endTime <= currentTime)
    )

    const getSeatInfo = (seatId: number): Partial<Seat> => seats.find((s: Seat) => s.id === seatId) || {}
    const getRoomInfo = (roomId: number | undefined): Partial<Room> => rooms.find((r: Room) => r.id === roomId) || {}
    ...
    const handleCancel = async () => {
        if (selectedReservationId) {
            try {
                ...
                addToast((error as Error).message || '取消失败', 'error')
            }
    ...
    const getStatusBadge = (reservation: SeatReservation) => {
        ...
        <div className="page-container">
            ...
            {activeTab === 'active' && (
                <div className="reservation-list">
                    {activeReservations.map((reservation: SeatReservation) => {
            ...
            {activeTab === 'history' && (
                <div className="data-table-container">
                    <table className="data-table">
                        ...
                        <tbody>
                            {historyReservations.map((reservation: SeatReservation) => {
                                const seat = getSeatInfo(reservation.seatId)
                                const room = getRoomInfo(seat.roomId)

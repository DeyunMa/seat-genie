...
import type { User, Book, SeatReservation, BookBorrowing, Notification } from '../../types'
...
function NotificationCenter() {
    ...
    const allNotifications = useMemo<Notification[]>(() => {
        ...
        if (isStaffOrAdmin) {
            const overdueBorrowings = bookBorrowings.filter((b: BookBorrowing) =>
                b.status === 'borrowed' && b.dueDate < today
            )
            overdueBorrowings.forEach((b: BookBorrowing) => {
                const borrower = users.find((u: User) => u.id === b.userId)
                const book = books.find((bk: Book) => bk.id === b.bookId)
                notifications.push({
                    id: `overdue-${b.id}`,
                    type: 'warning',
                    icon: '⚠️',
            ...
            const violatedReservations = seatReservations.filter((r: SeatReservation) => r.status === 'violated')
            violatedReservations.forEach((r: SeatReservation) => {
                const violator = users.find((u: User) => u.id === r.userId)
                notifications.push({
                    id: `violated-${r.id}`,
                    type: 'error',
                    icon: '🚫',
            ...
            const myBorrowings = bookBorrowings.filter((b: BookBorrowing) => b.userId === user?.id)

            const dueSoon = myBorrowings.filter((b: BookBorrowing) => {
            ...
            dueSoon.forEach((b: BookBorrowing) => {
                const book = books.find((bk: Book) => bk.id === b.bookId)
                const dueDate = new Date(b.dueDate)
                const todayDate = new Date(today)
            ...
            const myOverdue = myBorrowings.filter((b: BookBorrowing) => b.status === 'borrowed' && b.dueDate < today)
            myOverdue.forEach((b: BookBorrowing) => {
                const book = books.find((bk: Book) => bk.id === b.bookId)
                notifications.push({
                    id: `my-overdue-${b.id}`,
                    type: 'error',
            ...
            const todayReservations = seatReservations.filter((r: SeatReservation) =>
                r.userId === user?.id && r.date === today && r.status === 'active'
            )
            todayReservations.forEach((r: SeatReservation) => {
                notifications.push({
                    id: `today-${r.id}`,
                    type: 'info',
                    icon: '📅',

...
router.get("/:id", validate({ params: idSchema }), (req: Request, res: Response, next: NextFunction) => {
  try {
    const book = booksService.getBook((req.params as any).id);
    if (!book) {
...
router.post("/", validate({ body: bookSchema }), (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put("/:id", validate({ params: idSchema, body: bookSchema }), (req: Request, res: Response, next: NextFunction) => {
  try {
    const book = booksService.updateBook((req.params as any).id, req.body);
    if (!book) {
      throw new NotFoundError("Book not found");
    }
    res.json({ data: book });
...
router.delete("/:id", validate({ params: idSchema }), (req: Request, res: Response, next: NextFunction) => {
  try {
    const deleted = booksService.deleteBook((req.params as any).id);
    if (!deleted) {
      throw new NotFoundError("Book not found");
    }
    res.status(204).send();
